...
const searchWord = async () => {
  ...
  try {
    const wordResponse = await axiosInstance.get(`/search-word?word=${query.value}`);
    wordData.value = wordResponse.data;

    const exampleResponse = await axiosInstance.get(`/example-sentence?word=${query.value}`);
