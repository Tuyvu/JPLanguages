<template>
  ...
  <div class="container my-5">
    <Skit :conversations="lesson?.conversations || []" />
    <Vocabulary :vocabulary="lesson?.vocabulary || []" />
    <Key :keys="lesson?.keys || []" />
    <Kanji :kanjis="lesson?.kanjis || []" />
  </div>
</template>
...
const saveProgress = async (courseId,lessonId, timeWatched, userId) => {
  const payload = {
  ...
  try {
    await axiosInstance.post("/lesson/lessonUser", payload);
...
const markLessonCompleted = () => {
  ...
  saveProgress(courseId,lesson.value.id, watchedTime, userId);
};
