            ...
            <div class="col-md-8">
                ...
                <div class="box box-primary">
                    <div class="my-2 box-header with-border">
                        <h3 class="box-title">cập nhật bài kiểm tra</h3>
                    </div>
                    <!-- /.box-header -->
                    <!-- form start -->
                    <form role="form" method="POST" action="{{ route('tests.update', $tests->id) }}" enctype="multipart/form-data">
                      @method('PUT')  
                      @csrf
                      <div class="box-body">
                        <div class="row">
                            <div class="form-group @error('title') has-error @enderror">
                                <div class="col-md-6">
                                    ...
                                    <input type="text" class="form-control" id="productname" name="title" placeholder="tên bài kiểm tra" value="{{ old('title', $tests->title) }}">
                                    @error('title')
                        ...
                        <div class="form-group">
                            ...
                            <select name="course_id" id="course_id" class="form-control">
                                ...
                                <option value="{{ $item->id }}" {{ old('course_id', $tests->course_id) == $item->id ? 'selected' : '' }}>
                                    {{ $item->title }}
                                </option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group @error('total_questions') has-error @enderror">
                            ...
                            <input type="text" class="form-control" name="total_questions" placeholder="total_questions" value="{{ old('total_questions', $tests->total_questions) }}">
                        ...
                        <div class="form-group @error('time_limit') has-error @enderror">
                            ...
                            <input type="text" class="form-control" name="time_limit" value="{{ old('time_limit', $tests->time_limit) }}" placeholder="time_limit" onkeypress="return event.charCode >= 48 && event.charCode <= 57">
                            @error('time_limit')
                                <span class="help-block" style="color: red;">{{ $message }}</span>
                            @enderror
                        </div>
                        ...
                        <div class="form-group @error('description') has-error @enderror">
                            ...
                            <textarea class="form-control" name="description" id="editor">{{ old('description',$tests->description) }}</textarea>
                            @error('description')
                                <span class="help-block" style="color: red;">{{ $message }}</span>
                            @enderror
                        </div>
                        <div id="testcount-container">
                            ...
                            @foreach ($tests->test_questions as $index=>$item)
                            <div class="test-group">
                                <div class="row">
                                    <div>
                                        <label for="test_name">Câu hỏi {{$index+1}}</label>
                                        <input type="text" class="form-control" name="question[]" value="{{ old('question',$item->question) }}" placeholder="Câu hỏi">
                                    </div>
                                </div>
                                <div class="mx-2 row">
                                    <div class="col-md-6">
                                        <div class="row">
                                            <div class="col-md-6">
                                                ...
                                                <input type="text" class="form-control mb-2" name="option_a[]" value="{{ old('option_a',$item->option_a) }}" placeholder="Đáp án A">
                                            </div>
                                            <div class="col-md-6">
                                                <label for="test_stock">Đáp án B</label>
                                                <input type="text" class="form-control mb-2" name="option_b[]" value="{{ old('option_b',$item->option_b) }}" placeholder="Đáp án B">
                                            </div>
                                        </div>
                                    </div>
                                    <div class="col-md-6">
                                        <div class="row">
                                            <div class="col-md-6">
                                                ...
                                                <input type="text" class="form-control mb-2" name="option_c[]" value="{{ old('option_c',$item->option_c) }}" placeholder="Đáp án C">
                                            </div>
                                            <div class="col-md-6">
                                                <label for="test_stock">Đáp án D</label>
                                                <input type="text" class="form-control mb-2" name="option_d[]" value="{{ old('option_d',$item->option_d) }}" placeholder="Đáp án D">
                                            </div>
                                        </div>
                                    </div>
                                    <div class="col-md-6">
                                        <div class="row">
                                            <div class="col-md-6">
                                                ...
                                                <input type="text" class="form-control mb-2" name="correct_answer[]" value="{{ old('opcorrect_answerion_a',$item->correct_answer) }}" placeholder="Đáp án đúng">
                                            </div>
                                            <div class="col-md-6">
...
<script>
            ...
            document.querySelectorAll('.remove-test').forEach(button => {
                button.addEventListener('click', function() {
                    if (confirm('Bạn có chắc chắn muốn xóa câu hỏi này không?')) {
                        const testContainer = document.getElementById('testcount-container'); // Cập nhật ID phù hợp
                        const newtestGroup = this.closest('.test-group'); // Xác định phần tử cha cần xóa
